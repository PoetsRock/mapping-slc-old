...
"use strict";angular.module("core").controller("HomeController",["$scope","Authentication","ApiKeys","$http","MarkerDataService","mapService","AdminAuthService","$rootScope","$location","$sce","UtilsService",function($scope,Authentication,ApiKeys,$http,MarkerDataService,mapService,AdminAuthService,$rootScope,$location,$sce,UtilsService){$scope.authentication=Authentication,$scope.isAdmin=AdminAuthService,$scope.featuredProjects=[];var getFeatured=function(){$http.get("/api/v1/featured",{cache:!0}).then(function(resolved,rejected){$scope.featuredProjects=resolved.data,console.log("$scope.featuredProjects:\n",$scope.featuredProjects)})};getFeatured(),UtilsService.cssLayout(),$scope.trustAsHtml=$sce.trustAsHtml,$scope.goToProject=function(id){$location.path("projects/"+id)},$scope.photo0="chris--bw-2.jpg",$scope.photo1="as_thumb_150.jpg",$scope.photo2="wli_thumb_150.jpg",$scope.photo3="dw_thumb_150.jpg",$scope.photo4="as_thumb_bw.png",$scope.projectMarker=null,$scope.markerData=null,$scope.overlayActive=!0,$scope.sourceTo="",$scope.sourceFrom="",$scope.menuOpen=!1,$scope.shadeMap=!1,$scope.toggleOverlayFunction=function(sourceFrom,sourceTo){$scope.sourceFrom=sourceFrom,$scope.sourceTo=sourceTo,$scope.overlayActive&&"overlay"===sourceFrom?(console.log("toggle the shade! v1\n",$scope.overlayActive,"\n",sourceFrom),$scope.overlayActive=!$scope.overlayActive,$scope.shadeMap=!0):$scope.overlayActive&&"menu-closed"===sourceFrom?(console.log("toggle the shade! v2\n",$scope.overlayActive,"\n",sourceFrom),$scope.overlayActive=!1,$scope.menuOpen=!0,$scope.shadeMap=!0):$scope.overlayActive||"menu-closed"!==sourceFrom||$scope.menuOpen?$scope.overlayActive||"home"!==sourceFrom||(console.log("toggle the shade! v4\n",$scope.overlayActive,"\n",sourceFrom),$scope.menuOpen=!1,$scope.overlayActive=!0,$scope.shadeMap=!1):(console.log("toggle the shade! v3\n",$scope.overlayActive,"\n",sourceFrom),$scope.menuOpen=!$scope.menuOpen,$scope.shadeMap=!1)},$scope.attributionFull=!1,$scope.attributionText='<div style="padding: 0 5px 0 2px"><a href="http://www.mapbox.com/about/maps/" target="_blank">Mapbox</a>(the world\'s best maps) & <a href="http://leafletjs.com/" target="_blank">Leaflet</a>, with map data by <a href="http://openstreetmap.org/copyright">OpenStreetMap©</a> | <a href="http://mapbox.com/map-feedback/" class="mapbox-improve-map">Improve this map</a></div>',$scope.markers=!0,$scope.filters=!0,$scope.censusDataTractLayer=!0,$scope.googlePlacesLayer=!1,$scope.sidebarToggle=!1,ApiKeys.getApiKeys().then(function(resolved,rejected){mapFunction(resolved.data.MAPBOX_KEY,resolved.data.MAPBOX_SECRET)});var mapFunction=function(mapboxKey,mapboxAccessToken){L.mapbox.accessToken=mapboxAccessToken;var map=(document.getElementById("info"),L.mapbox.map("map",null,{infoControl:!1,attributionControl:!1,legendControl:{position:"bottomleft"}}).setView([40.7630772,-111.8689467],12).addControl(L.control.zoom({position:"topright"}))),grayMap=L.mapbox.tileLayer("poetsrock.b06189bb"),mainMap=L.mapbox.tileLayer("poetsrock.la999il2"),topoMap=L.mapbox.tileLayer("poetsrock.la97f747"),greenMap=L.mapbox.tileLayer("poetsrock.jdgpalp2"),landscape=L.tileLayer("http://{s}.tile.thunderforest.com/landscape/{z}/{x}/{y}.png"),comic=L.mapbox.tileLayer("poetsrock.23d30eb5"),watercolor=L.tileLayer("https://stamen-tiles-{s}.a.ssl.fastly.net/watercolor/{z}/{x}/{y}.png"),layers={"Main Map":mainMap,"Topo Map":topoMap,"Green Map":greenMap,Landscape:landscape,"Comically Yours":comic,"Gray Day":grayMap,Watercolor:watercolor};mainMap.addTo(map),L.control.layers(layers).addTo(map);var legend='<div style="padding: 0 5px 0 2px"><a href="http://www.mapbox.com/about/maps/" target="_blank">Mapbox</a>(the world\'s best maps) & <a href="http://leafletjs.com/" target="_blank">Leaflet</a>, with map data by <a href="http://openstreetmap.org/copyright">OpenStreetMap©</a> | <a href="http://mapbox.com/map-feedback/" class="mapbox-improve-map">Improve this map</a></div>';map.getContainer().querySelector("#legend").onclick=function(){return"active"===this.className?(map.legendControl.removeLegend(legend),this.className=""):(map.legendControl.addLegend(legend),this.className="active"),!1},MarkerDataService.getMarkerData().success(function(markerData){$scope.addProjectMarkers(markerData)}).error(function(data,status){alert("Failed to load project markers. Status: "+status)}),$scope.markerArray=[],$scope.addProjectMarkers=function(markerData){$scope.markerData=markerData;var index=0;for(var prop in markerData)$scope.projectMarker=L.mapbox.featureLayer({type:"Feature",geometry:{type:"Point",coordinates:[markerData[prop].lng,markerData[prop].lat]},properties:{"marker-size":"large","marker-color":markerData[prop].markerColor,"marker-symbol":"marker-stroked",projectId:markerData[prop]._id,summary:markerData[prop].storySummary,title:markerData[prop].title,mainImage:markerData[prop].mainImage,category:markerData[prop].category,mapImage:markerData[prop].mapImage,lat:markerData[prop].lat,lng:markerData[prop].lng,published:markerData[prop].createdOn,leafletId:null,arrayIndexId:index}}).on("click",function(e){return $scope.$apply(function(){$scope.storyEvent=e.target._geojson.properties}),map.panTo(e.layer.getLatLng()),popupMenuToggle(e),$scope.projectMarker[prop]}),$scope.projectMarker.addTo(map),$scope.markerArray.push($scope.projectMarker),index++;return $scope.markerArray};new L.Point(30,-16);$scope.toggleGooglePlacesData=function(){$scope.googlePlacesLayer?map.removeLayer(googlePlacesMarkerLayer):map.addLayer(googlePlacesMarkerLayer)},map.on("click",function(e){console.log("click event",e),$scope.menuOpen?($scope.sidebar.close(),$scope.shadeMap=!1):(console.log("map click!"),$scope.overlayActive=!1)}),$scope.getProjectMarkers=function(markerData){}},popupIndex=0,popupMenuToggle=function(e){$scope.menuOpen||popupIndex===e.target._leaflet_id?($scope.menuOpen||popupIndex!==e.target._leaflet_id)&&($scope.menuOpen&&popupIndex!==e.target._leaflet_id?($scope.sidebar.open("details"),popupIndex=e.target._leaflet_id):$scope.menuOpen&&popupIndex===e.target._leaflet_id&&($scope.sidebar.close(),popupIndex=0)):($scope.toggleOverlayFunction("menu-closed"),$scope.sidebar.open("details"),popupIndex=e.target._leaflet_id)}}]);
"use strict";angular.module("core").controller("contentBodyController",["MenuService","$scope",function(MenuService,$scope){return $scope.isMenuClosed=!MenuService.open.all,$scope.closeMenu=function(){MenuService.setShowAll(!1)},$scope.$on("MenuService.update",function(event,open){$scope.isMenuClosed=!open.all}),$scope}]).controller("chatController",["MenuService","$scope",function(MenuService,$scope){return $scope.title="Chat",$scope.userImage="./img/defaultPhoto.png",$scope.chatSide=!MenuService.open.chatSide,$scope.chatSideChangeState=function(){MenuService.setChatSide(!$scope.chatSide)},$scope.$on("MenuService.chatChangedState",function(event,open){$scope.chatSide=open.chatSideClosed}),$scope}]).service("MenuService",["$rootScope",function($rootScope){return{itemsMenu:[{name:"Home",href:"#home",icon:"fa-home"},{name:"Setting",href:"#setting",icon:"fa-cogs"},{name:"Portfolio",href:"#portfolio",icon:"fa-desktop",children:[{name:"child 1",href:"#child_1",icon:"fa-sort-amount-asc"},{name:"child 2",href:"#child_2",icon:"fa-sort-amount-asc"}]},{name:"Blog",href:"#blog",icon:"fa-book"},{name:"Prices",href:"#prices",icon:"fa-eur",children:[{name:"child 3",href:"#child_3",icon:"fa-sort-amount-asc"},{name:"child 4",href:"#child_4",icon:"fa-sort-amount-asc"}]}],open:{part:!1,all:!1,chatSideClosed:!1},setShowPart:function(val){this.open.part=val,$rootScope.$broadcast("MenuService.update",this.open)},setShowAll:function(val){this.open.all=val,val&&(this.open.part=!1),$rootScope.$broadcast("MenuService.update",this.open)},setChatSide:function(val){this.open.chatSideClosed=val,$rootScope.$broadcast("MenuService.chatChangedState",this.open)}}}]).directive("leftMenu",["MenuService",function(MenuService){return{restrict:"EA",templateUrl:"modules/core/client/directives/views/google-left-menu.client.view.html",replace:!0,link:function(scope,elm,attrs){scope.itemsMenu=MenuService.itemsMenu,scope.showAll=MenuService.open.all,scope.showPart=MenuService.open.part,scope.mouseenter=function(){MenuService.setShowPart(!0)},scope.mouseleave=function(){MenuService.setShowPart(!1)},scope.menuHover=function(event){MenuService.setShowAll(!0)},scope.menuTogle=function(event){"triggerMenu"===event.target.id&&(event.stopPropagation(),MenuService.setShowAll(!scope.showAll))},scope.$on("MenuService.update",function(event,open){scope.showAll=open.all,scope.showPart=open.part})}}}]);
...
"use strict";angular.module("core").directive("featuredProjects",function(){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/featured-projects.html",controller:["$scope","$http",function($scope,$http){$scope.showTitle=!1}]}});
...
>>>>>>> f8a0babcbf34a835a01ec7c6b21e8d9e22f1968b
