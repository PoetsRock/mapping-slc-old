...
"use strict";angular.module("admins").directive("projectAdminFeatures",function(){return{restrict:"E",templateUrl:"/modules/admins/client/directives/views/project-admin-features.html",link:function(scope,element,attrs,modelCtrl){"published"===scope.project.status&&console.log("pub be hooked, yo, up yeah up")}}});
...
"use strict";angular.module("core").directive("footerDirective",["UtilsService",function(UtilsService){return{restrict:"AE",priority:0,templateUrl:"/modules/core/client/directives/views/footer-directive.html",controller:function(){UtilsService.cssLayout()}}}]);
...
"use strict";angular.module("core").directive("mainMenu",["AdminAuthService",function(AdminAuthService){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/main-menu.html",controller:["$scope",function($scope){$scope.isAdmin=AdminAuthService.user}],link:function(){L.control.sidebar("sidebar",{closeButton:!0,position:"left"}).addTo(map)}}}]);
...
"use strict";angular.module("core").directive("modalDirective",function(){return{restrict:"E",link:function(){}}});
...
"use strict";angular.module("core").service("FullScreenService",[,function(){this.fullScreen=function(){}}]);
...
"use strict";angular.module("users").controller("UserController",["$scope","$state","$stateParams","Authentication","UserData","Users",function($scope,$state,$stateParams,Authentication,UserData,Users){$scope.authentication=Authentication,$scope.user=Authentication.user,$scope.remove=function(user){confirm("Are you sure you want to delete this user?")&&(user?(user.$remove(),$scope.users.splice($scope.users.indexOf(user),1)):$scope.user.$remove(function(){$state.go("admin.users")}))},$scope.update=function(isValid){return isValid?void user.$update(function(){$state.go("admin.user",{userId:user._id})},function(errorResponse){$scope.error=errorResponse.data.message}):($scope.$broadcast("show-errors-check-validity","userForm"),!1)},$scope.find=function(){$scope.users=Users.query($scope.query)},$scope.findOne=function(){$scope.user=UserData.get({userId:$stateParams.userId}),console.log("$scope.users: ",$scope.users)}}]);
...
"use strict";angular.module("users").factory("AdminAuthService",["$window","Authentication",function($window,Authentication){if(""!==Authentication.user){var isAdmin={user:$window.user.roles[0]};return console.log("isAdmin.user",isAdmin.user),isAdmin}return isAdmin={user:"notAdmin"},console.log("!isAdmin.user",isAdmin.user),isAdmin}]);
...
"use strict";angular.module("users.admin").controller("UserListController",["$scope","$filter","AdminUpdateUser",function($scope,$filter,AdminUpdateUser){AdminUpdateUser.query(function(data){$scope.users=data,$scope.buildPager()}),$scope.buildPager=function(){$scope.pagedItems=[],$scope.itemsPerPage=15,$scope.currentPage=1,$scope.figureOutItemsToDisplay()},$scope.figureOutItemsToDisplay=function(){$scope.filteredItems=$filter("filter")($scope.users,{$:$scope.search}),$scope.filterLength=$scope.filteredItems.length;var begin=($scope.currentPage-1)*$scope.itemsPerPage,end=begin+$scope.itemsPerPage;$scope.pagedItems=$scope.filteredItems.slice(begin,end)},$scope.pageChanged=function(){$scope.figureOutItemsToDisplay()}}]);
...
"use strict";angular.module("users").controller("ChangeProfilePictureController",["$scope","$timeout","$window","Authentication","FileUploader",function($scope,$timeout,$window,Authentication,FileUploader){$scope.user=Authentication.user,$scope.imageURL=$scope.user.profileImageURL,$scope.uploader=new FileUploader({url:"api/v1/users/picture"}),$scope.uploadToCloud=new FileUploader({url:"/api/v1/users/upload"}),$scope.uploadToCloud.filters.push({name:"imageFilter",fn:function(item,options){var type="|"+item.type.slice(item.type.lastIndexOf("/")+1)+"|";return-1!=="|jpg|png|jpeg|bmp|gif|".indexOf(type)}}),$scope.uploadToCloud.onAfterAddingFile=function(fileItem){if($window.FileReader){var fileReader=new FileReader;fileReader.readAsDataURL(fileItem._file),fileReader.onload=function(fileReaderEvent){$timeout(function(){$scope.imageURL=fileReaderEvent.target.result},0)}}},$scope.uploadToCloud.onSuccessItem=function(fileItem,response,status,headers){$scope.success=!0,$scope.user=Authentication.user=response,$scope.cancelUpload()},$scope.uploadToCloud.onErrorItem=function(fileItem,response,status,headers){$scope.cancelUpload(),$scope.error=response.message},$scope.uploadProfilePicture=function(){$scope.success=$scope.error=null,$scope.uploadToCloud.uploadAll()},$scope.cancelUpload=function(){$scope.uploadToCloud.clearQueue(),$scope.imageURL=$scope.user.profileImageURL}}]);
"use strict";angular.module("users").controller("EditProfileController",["$scope","$http","$location","Users","UserData","$stateParams","Authentication","AdminAuthService","UtilsService",function($scope,$http,$location,Users,UserData,$stateParams,Authentication,AdminAuthService,UtilsService){$scope.user=Authentication.user,$scope.isAdmin=AdminAuthService,UtilsService.cssLayout(),$scope.update=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","userAdminForm"),!1;var userToEdit=$scope.userToEdit;userToEdit.$update(function(){$location.path("users/"+user._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.toggleEdit=!1,$scope.toggleId=0,$scope.toggleEditFn=function(editNum){$scope.toggleEdit=!$scope.toggle,$scope.toggleId=editNum},$scope.find=function(){$scope.users=Users.query()},$scope.find=function(){$scope.users=Users.query($scope.query)},$scope.findOne=function(){console.log("$stateParams.userId",$stateParams.userId),$scope.userToEdit=UserData.get({userId:$stateParams.userId}),console.log("$scope.userToEdit: ",$scope.userToEdit)}}]);
