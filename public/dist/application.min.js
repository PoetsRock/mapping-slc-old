...
"use strict";angular.module("core").directive("moveMap",function(){return{restrict:"A",link:function(scope,element){scope.$on("CurrentStory",function(event,data){data&&element.css({position:"absolute",left:"-19%"})}),scope.$on("closeMap",function(){element.css({position:"absolute",left:"0"})})}}});
"use strict";angular.module("core").directive("featureSideBar",function(){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/projects-sidebar.html",scope:!0,link:function(scope){function identical(array){for(var i=0;i<array.length-1;i++)if(array[i]!=array[i+1])return!1;return!0}scope.show=!1,scope.hide=function(){scope.show=!scope.show,scope.$emit("closeMap")},scope.eyedees=[],scope.$on("CurrentStory",function(event,data){scope.project=data,scope.eyedees.push(scope.project.projectId),1==scope.eyedees.length&&(scope.show=!0),scope.eyedees.length>1&&(identical(scope.eyedees)?(scope.hide(),scope.eyedees=[]):(scope.show=!0,scope.eyedees=[]))})}}});
