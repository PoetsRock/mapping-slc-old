...
"use strict";angular.module("angular-redactor").controller("RedactorController",["$scope","$stateParams","Authentication","$http","AdminAuthService",function($scope,$stateParams,Authentication,$http,AdminAuthService){$scope.authentication=Authentication,$scope.isAdmin=AdminAuthService,$scope.redactorOptions={},$scope.changeContent=function(){$scope.content="<h1>Some bogus content</h1>"},$scope.content="<p>This is my awesome content</p>",$scope.user=Authentication.user,$scope.uploading=!1,$scope.imageURL=$scope.user.profileImageURL;$scope.onFileSelect=function(files){if(files.length>0){$scope.uploading=!0;var filename=files[0].name,type=files[0].type,query={user:$scope.user,filename:filename,type:type},configObj={cache:!0};$http.post("api/v1/s3/upload",query,configObj).success(function(result){console.log("result v1\n",result),Upload.upload({url:result.url,transformRequest:function(data,headersGetter){var headers=headersGetter();return delete headers.Authorization,console.log("data v1\n",data),data},fields:result.fields,method:"POST",file:files[0]}).progress(function(evt){console.log("progress: "+parseInt(100*evt.loaded/evt.total))}).success(function(data,status,headers,config){var s3Result=xmlToJSON.parseString(data);$scope.uploading=!1,console.log("s3Result:::\n",s3Result),console.log("status: ",status),console.log("The file "+config.file.name+" is uploaded successfully.\nResponse:\n",s3Result),$scope.user.profileImageURL="https://s3.amazonaws.com"+s3Result.PostResponse[0].Bucket[0]._text+"/"+s3Result.PostResponse[0].Key[0]._text,console.log("https://s3.amazonaws.com"+s3Result.PostResponse[0].Bucket[0]._text+"/"+s3Result.PostResponse[0].Key[0]._text),console.log("$scope.user.profileImageURL final:\n",user.profileImageURL)})}).error(function(data,status,headers,config){$scope.uploading=!1})}};({user:$scope.user})}]);
"use strict";!function(){var redactorOptions={plugins:["source","alignment","filemanager","imagemanager","video"],focus:!0};angular.module("angular-redactor").constant("redactorOptions",redactorOptions).directive("redactor",["$timeout",function($timeout){return{restrict:"A",require:"ngModel",link:function(scope,element,attrs,ngModel){scope.redactorLoaded=!1;var editor,updateModel=function(value){$timeout(function(){scope.$apply(function(){ngModel.$setViewValue(value)})})},options={callbacks:{change:updateModel}},additionalOptions=attrs.redactor?scope.$eval(attrs.redactor):{};angular.extend(options,redactorOptions,additionalOptions),console.log(":::options::::\n",options),$timeout(function(){editor=element.redactor(options),console.log(":::editor::::\n",editor),ngModel.$render()}),ngModel.$render=function(){angular.isDefined(editor)&&$timeout(function(){element.redactor("code.set",ngModel.$viewValue||""),scope.redactorLoaded=!0})}}}}])}();
!function($){$.Redactor.prototype.filemanager=function(){return{langs:{en:{upload:"Upload",choose:"Choose"}},init:function(){this.opts.fileManagerJson&&this.modal.addCallback("file",this.filemanager.load)},load:function(){var $box=$('<div  style="overflow: auto; height: 300px; display: none;" class="redactor-modal-tab" data-title="Choose">').hide();this.modal.getModal().append($box),$.ajax({dataType:"json",cache:!1,url:this.opts.fileManagerJson,success:$.proxy(function(data){var ul=$('<ul id="redactor-modal-list">');$.each(data,$.proxy(function(key,val){var a=$('<a href="#" data-params="'+encodeURI(JSON.stringify(val))+'" class="redactor-file-manager-link">'+val.title+' <span style="font-size: 11px; color: #888;">'+val.name+'</span> <span style="position: absolute; right: 10px; font-size: 11px; color: #888;">('+val.size+")</span></a>"),li=$("<li />");a.on("click",$.proxy(this.filemanager.insert,this)),li.append(a),ul.append(li)},this)),$box.append(ul)},this)})},insert:function(e){e.preventDefault();var $el=$(e.target).closest(".redactor-file-manager-link"),json=$.parseJSON(decodeURI($el.attr("data-params")));this.file.insert(json,null)}}}}(jQuery);
...
!function($){$.Redactor.prototype.source=function(){return{init:function(){var button=this.button.addFirst("html","HTML");this.button.addCallback(button,this.source.toggle);var style={width:"100%",margin:"0",background:"#111","box-sizing":"border-box",color:"rgba(255, 255, 255, .8)","font-size":"14px",outline:"none",padding:"16px","line-height":"22px","font-family":'Menlo, Monaco, Consolas, "Courier New", monospace'};this.source.$textarea=$("<textarea />"),this.source.$textarea.css(style).hide(),"textarea"===this.opts.type?this.core.box().append(this.source.$textarea):this.core.box().after(this.source.$textarea),this.core.element().on("destroy.callback.redactor",$.proxy(function(){this.source.$textarea.remove()},this))},toggle:function(){return this.source.$textarea.hasClass("open")?this.source.hide():this.source.show()},setCaretOnShow:function(){this.source.offset=this.offset.get();$(window).scrollTop(),this.core.editor().innerWidth(),this.core.editor().innerHeight();this.source.start=0,this.source.end=0;var $editorDiv=$("<div/>").append($.parseHTML(this.core.editor().html(),document,!0)),$selectionMarkers=$editorDiv.find("span.redactor-selection-marker");if($selectionMarkers.length>0){var editorHtml=$editorDiv.html().replace(/&amp;/g,"&");1===$selectionMarkers.length?(this.source.start=this.utils.strpos(editorHtml,$editorDiv.find("#selection-marker-1").prop("outerHTML")),this.source.end=this.source.start):2===$selectionMarkers.length&&(this.source.start=this.utils.strpos(editorHtml,$editorDiv.find("#selection-marker-1").prop("outerHTML")),this.source.end=this.utils.strpos(editorHtml,$editorDiv.find("#selection-marker-2").prop("outerHTML"))-$editorDiv.find("#selection-marker-1").prop("outerHTML").toString().length)}},setCaretOnHide:function(html){if(this.source.start=this.source.$textarea.get(0).selectionStart,this.source.end=this.source.$textarea.get(0).selectionEnd,this.source.start>this.source.end&&this.source.end>0){var tempStart=this.source.end,tempEnd=this.source.start;this.source.start=tempStart,this.source.end=tempEnd}if(this.source.start=this.source.enlargeOffset(html,this.source.start),this.source.end=this.source.enlargeOffset(html,this.source.end),html=html.substr(0,this.source.start)+this.marker.html(1)+html.substr(this.source.start),this.source.end>this.source.start){var markerLength=this.marker.html(1).toString().length;html=html.substr(0,this.source.end+markerLength)+this.marker.html(2)+html.substr(this.source.end+markerLength)}return html},hide:function(){this.source.$textarea.removeClass("open").hide(),this.source.$textarea.off(".redactor-source");var code=this.source.$textarea.val();code=this.paragraphize.load(code),code=this.source.setCaretOnHide(code),this.code.start(code),this.button.enableAll(),this.core.editor().show().focus(),this.selection.restore(),this.code.sync()},show:function(){this.selection.save(),this.source.setCaretOnShow();var height=this.core.editor().innerHeight(),code=this.code.get();code=code.replace(/\n\n\n/g,"\n"),code=code.replace(/\n\n/g,"\n"),this.core.editor().hide(),this.button.disableAll("html"),this.source.$textarea.val(code).height(height).addClass("open").show(),this.source.$textarea.on("keyup.redactor-source",$.proxy(function(){"textarea"===this.opts.type&&this.core.textarea().val(this.source.$textarea.val())},this)),this.marker.remove(),$(window).scrollTop(scroll),this.source.$textarea[0].setSelectionRange&&this.source.$textarea[0].setSelectionRange(this.source.start,this.source.end),this.source.$textarea[0].scrollTop=0,setTimeout($.proxy(function(){this.source.$textarea.focus()},this),0)},enlargeOffset:function(html,offset){var htmlLength=html.length,c=0;if(">"===html[offset])c++;else for(var i=offset;htmlLength>=i&&(c++,">"!==html[i]);i++)if("<"===html[i]||i===htmlLength){c=0;break}return offset+c}}}}(jQuery);
...
"use strict";angular.module("projects").directive("projectUploaderDirective",function(){return{restrict:"AE",templateUrl:"/modules/projects/client/directives/views/projectUploader.html",controller:["$scope","$http","Authentication",function($scope,$http,Authentication){}]}});
