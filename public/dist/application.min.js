...
"use strict";angular.module("admins").directive("projectAdminFeatures",function(){return{restrict:"EA",templateUrl:"/modules/admins/client/directives/views/project-admin-features.html",link:function($scope){console.log("inside directive projAdminFeatures, logging $scope.project:\n",$scope.project),"published"===$scope.project.status}}});
...
"use strict";angular.module("core").controller("HomeController",["$scope","Authentication","ApiKeys","$http","MarkerDataService","mapService","AdminAuthService","$rootScope","$location","$sce","UtilsService","MenuService",function($scope,Authentication,ApiKeys,$http,MarkerDataService,mapService,AdminAuthService,$rootScope,$location,$sce,UtilsService,MenuService){$scope.authentication=Authentication,$scope.isAdmin=AdminAuthService,$scope.featuredProjects=[],$scope.featuredProjects=[];var getFeatured=function(){$http.get("/api/v1/featured",{cache:!0}).then(function(resolved,rejected){$scope.featuredProjects=resolved.data,console.log("$scope.featuredProjects:\n",$scope.featuredProjects)})};getFeatured(),UtilsService.cssLayout(),$scope.trustAsHtml=$sce.trustAsHtml,$scope.goToProject=function(id){$location.path("projects/"+id)},$scope.projectMarker=null,$scope.markerData=null,$scope.overlayActive=!0,$scope.sourceTo="",$scope.sourceFrom="",$scope.menuOpen=!1,$scope.shadeMap=!1,$scope.toggleOverlayFunction=function(sourceFrom,sourceTo){$scope.sourceFrom=sourceFrom,$scope.sourceTo=sourceTo,$scope.overlayActive&&"overlay"===sourceFrom?(console.log("toggle the shade! v1\n",$scope.overlayActive,"\n",sourceFrom),$scope.overlayActive=!$scope.overlayActive,$scope.shadeMap=!0):$scope.overlayActive&&"menu-closed"===sourceFrom?(console.log("toggle the shade! v2\n",$scope.overlayActive,"\n",sourceFrom),$scope.overlayActive=!1,$scope.menuOpen=!0,$scope.shadeMap=!0):$scope.overlayActive||"menu-closed"!==sourceFrom||$scope.menuOpen?$scope.overlayActive||"home"!==sourceFrom||(console.log("toggle the shade! v4\n",$scope.overlayActive,"\n",sourceFrom),$scope.menuOpen=!1,$scope.overlayActive=!0,$scope.shadeMap=!1,MenuService.setShowAll(!1),MenuService.setShowPart(!1)):(console.log("toggle the shade! v3\n",$scope.overlayActive,"\n",sourceFrom),$scope.menuOpen=!$scope.menuOpen,$scope.shadeMap=!1,MenuService.setShowAll(!1),MenuService.setShowPart(!1))},$scope.attributionFull=!1,$scope.attributionText='<div style="padding: 0 5px 0 2px"><a href="http://www.mapbox.com/about/maps/" target="_blank">Mapbox</a>(the world\'s best maps) & <a href="http://leafletjs.com/" target="_blank">Leaflet</a>, with map data by <a href="http://openstreetmap.org/copyright">OpenStreetMap©</a> | <a href="http://mapbox.com/map-feedback/" class="mapbox-improve-map">Improve this map</a></div>',$scope.markers=!0,$scope.filters=!0,$scope.censusDataTractLayer=!0,$scope.googlePlacesLayer=!1,$scope.sidebarToggle=!1,ApiKeys.getApiKeys().then(function(resolved,rejected){mapFunction(resolved.data.MAPBOX_KEY,resolved.data.MAPBOX_SECRET)});var mapFunction=function(mapboxKey,mapboxAccessToken){L.mapbox.accessToken=mapboxAccessToken;var map=(document.getElementById("info"),L.mapbox.map("map",null,{infoControl:!1,attributionControl:!1,legendControl:{position:"bottomleft"}}).on("click",function(e){$scope.showAll||$scope.showPart?(console.log("if menu open, map click! event",e),MenuService.setShowAll(!1),$scope.shadeMap=!1):(console.log("else map click! event",e),MenuService.setShowAll(!1))}).setView([40.7630772,-111.8689467],12).addControl(L.control.zoom({position:"topright"}))),grayMap=L.mapbox.tileLayer("poetsrock.b06189bb"),mainMap=L.mapbox.tileLayer("poetsrock.la999il2"),topoMap=L.mapbox.tileLayer("poetsrock.la97f747"),greenMap=L.mapbox.tileLayer("poetsrock.jdgpalp2"),landscape=L.tileLayer("http://{s}.tile.thunderforest.com/landscape/{z}/{x}/{y}.png"),comic=L.mapbox.tileLayer("poetsrock.23d30eb5"),watercolor=L.tileLayer("https://stamen-tiles-{s}.a.ssl.fastly.net/watercolor/{z}/{x}/{y}.png"),layers={"Main Map":mainMap,"Topo Map":topoMap,"Green Map":greenMap,Landscape:landscape,"Comically Yours":comic,"Gray Day":grayMap,Watercolor:watercolor};mainMap.addTo(map),L.control.layers(layers).addTo(map);var legend='<div style="padding: 0 5px 0 2px"><a href="http://www.mapbox.com/about/maps/" target="_blank">Mapbox</a>(the world\'s best maps) & <a href="http://leafletjs.com/" target="_blank">Leaflet</a>, with map data by <a href="http://openstreetmap.org/copyright">OpenStreetMap©</a> | <a href="http://mapbox.com/map-feedback/" class="mapbox-improve-map">Improve this map</a></div>';map.getContainer().querySelector("#legend").onclick=function(){return"active"===this.className?(map.legendControl.removeLegend(legend),this.className=""):(map.legendControl.addLegend(legend),this.className="active"),!1},MarkerDataService.getMarkerData().success(function(markerData){$scope.addProjectMarkers(markerData)}).error(function(data,status){alert("Failed to load project markers. Status: "+status)}),$scope.markerArray=[],$scope.addProjectMarkers=function(markerData){$scope.markerData=markerData;var index=0;for(var prop in markerData)$scope.projectMarker=L.mapbox.featureLayer({type:"Feature",geometry:{type:"Point",coordinates:[markerData[prop].lng,markerData[prop].lat]},properties:{"marker-size":"large","marker-color":markerData[prop].markerColor,"marker-symbol":"marker-stroked",projectId:markerData[prop]._id,summary:markerData[prop].storySummary,title:markerData[prop].title,mainImage:markerData[prop].mainImage,category:markerData[prop].category,mapImage:markerData[prop].mapImage,mainImgThumbnail:markerData[prop].mainImgThumbnail,lat:markerData[prop].lat,lng:markerData[prop].lng,published:markerData[prop].createdOn,leafletId:null,arrayIndexId:index}}).on("click",function(e){return $scope.$apply(function(){console.log("on click, `e`:\n",e,"\n\n"),$scope.storyEvent=e.target._geojson.properties,$scope.$broadcast("CurrentStorty",$scope.storyEvent),console.log("on click `$scope.storyEvent`:\n",$scope.storyEvent,"\n\n")}),map.panTo(e.layer.getLatLng()),popupMenuToggle(e),$scope.projectMarker[prop]}),$scope.projectMarker.addTo(map),$scope.markerArray.push($scope.projectMarker),index++;return $scope.markerArray};new L.Point(30,-16);$scope.toggleGooglePlacesData=function(){$scope.googlePlacesLayer?map.removeLayer(googlePlacesMarkerLayer):map.addLayer(googlePlacesMarkerLayer)},$scope.getProjectMarkers=function(markerData){}};$scope.toggleSidebar=!1;var popupIndex=0,popupMenuToggle=function(e){$scope.menuOpen||popupIndex===e.target._leaflet_id?($scope.menuOpen||popupIndex!==e.target._leaflet_id)&&($scope.menuOpen&&popupIndex!==e.target._leaflet_id?popupIndex=e.target._leaflet_id:$scope.menuOpen&&popupIndex===e.target._leaflet_id&&($scope.sidebar.close(),popupIndex=0)):($scope.toggleOverlayFunction("menu-closed"),popupIndex=e.target._leaflet_id,$scope.toggleSidebar=!0)}}]);
"use strict";angular.module("core").controller("ModalController",["$scope","$uibModalInstance","items",function($scope,$uibModalInstance,items){$scope.items=items,$scope.selected={item:$scope.items[0],toStateUrl:items.toStateUrl},console.log("$scope.selected",$scope.selected),$scope.selected.item?(console.log("$scope.selected.item",$scope.selected.item),$scope.ok=function(){$uibModalInstance.close($scope.selected.item)}):(console.log("$scope.selected.toStateName",$scope.selected.toStateUrl),$scope.ok=function(){$uibModalInstance.close($scope.selected.toStateUrl)}),$scope.cancel=function(){$uibModalInstance.dismiss("user cancelled modal")}}]);
...
"use strict";angular.module("core").directive("mainMenu",["AdminAuthService",function(AdminAuthService){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/main-menu.html",controller:["$scope",function($scope){$scope.isAdmin=AdminAuthService.user,$scope.sidebar=L.control.sidebar("sidebar",{closeButton:!0,position:"left"}).addTo(map),$scope.sidebar.click=function(){L.DomUtil.hasClass(this,"active")?($scope.sidebar.close(),console.log("here i am close")):($scope.sidebar.open(this.firstChild.hash.slice(1)),console.log("here i am open"))},$scope.sidebar.on("click",function(e){console.log("sidebar click event",e)})}],link:function($scope){}}}]);
"use strict";angular.module("core").directive("mainMenuNew",["AdminAuthService",function(AdminAuthService){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/main-menu-new.html",controller:["$scope",function($scope){$scope.isAdmin=AdminAuthService.user,$scope.sidebar=L.control.sidebar("sidebar",{closeButton:!0,position:"left"}).addTo(map),$scope.sidebar.click=function(){L.DomUtil.hasClass(this,"active")?($scope.sidebar.close(),console.log("here i am close")):($scope.sidebar.open(this.firstChild.hash.slice(1)),console.log("here i am open"))},$scope.sidebar.on("click",function(e){console.log("sidebar click event",e)})}],link:function($scope){}}}]);
...
"use strict";angular.module("core").directive("leftMenu",["MenuService",function(MenuService){return{restrict:"EA",templateUrl:"modules/core/client/directives/views/google-left-menu.client.view.html",replace:!0,link:function(scope,elm,attrs){scope.itemsMenu=MenuService.itemsMenu,scope.showAll=MenuService.open.all,scope.showPart=MenuService.open.part,scope.showNone=MenuService.open.none,scope.mouseenter=function(){MenuService.setShowPart(!0)},scope.mouseleave=function(){MenuService.setShowPart(!1)},scope.menuHover=function(event){MenuService.setShowAll(!0),scope.toggleOverlayFunction("overlay")},scope.menuToggle=function(event){return"triggerMenu"!==event.target.id?void console.log("menuToggle inside `if`"):(event.stopPropagation(),void MenuService.setShowAll(!scope.showAll))},scope.$on("MenuService.update",function(event,open){scope.showAll=open.all,scope.showPart=open.part,scope.showNone=open.none})}}}]);
angular.module("core").directive("moveMap",["$document",function($document){"use strict";return{restrict:"A",link:function(scope,element,attr){scope.$on("CurrentStorty",function(event,data){data&&element.css({position:"absolute",left:"-19%"})}),scope.$on("closeMap",function(e,d){element.css({position:"absolute",left:"0"})})}}}]);
angular.module("core").directive("featureSideBar",["$document",function($document){"use strict";return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/projects-sidebar.html",scope:!0,link:function(scope,element,attr){scope.show=!1,scope.$on("CurrentStorty",function(event,data){scope.project=data,scope.show=!0,console.log("Project DATA: ",data),scope.hide=function(){scope.show=!scope.show,scope.$emit("closeMap")}})}}}]);
...
"use strict";angular.module("core").directive("signInDirective",function(){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/sign-in-directive.html",controller:["$scope","$http","Authentication",function($scope,$http,Authentication){return $scope.user=Authentication.user,""===$scope.user?void console.log("directive profilePic Service - calling nothing, just `return`"):void("default.png"===Authentication.user.profileImageFileName||""===Authentication.user.profileImageFileName?$scope.user.profileImage="modules/users/client/img/profile/default.png":""!==Authentication.user.profileImageFileName&&($scope.user.profileImage="modules/users/client/img/profile/uploads/uploaded-profile-image.jpg"))}]}});
...
"use strict";angular.module("core").service("MenuService",["$rootScope",function($rootScope){return{open:{part:!1,all:!1,none:!1,chatSideClosed:!1},setShowPart:function(val){this.open.part=val,$rootScope.$broadcast("MenuService.update",this.open)},setShowAll:function(val){this.open.all=val,val&&(this.open.part=!1,this.open.none=!1),$rootScope.$broadcast("MenuService.update",this.open)},setShowNone:function(val){console.log("setShowNone\nval\n",val,"\n\n"),this.open.none=val,val&&(this.open.part=!1,this.open.all=!1),$rootScope.$broadcast("MenuService.update",this.open)},setChatSide:function(val){this.open.chatSideClosed=val,$rootScope.$broadcast("MenuService.chatChangedState",this.open)}}}]);
...
"use strict";angular.module("projects").service("getUserFavorites",["$scope","$http",function($scope,$http){this.getUserFavoriteStories=function(userFavoriteProjects,projectId){userFavoriteProjects.forEach(function(userFavoriteProject){userFavoriteProject===projectId&&($scope.isFavorite=!0)})},this.toggleFavProject=function(){$scope.isFavorite=!$scope.isFavorite;var updateFavoriteObj={favorite:$scope.project.id,isFavorite:!0};$scope.isFavorite||(updateFavoriteObj.isFavorite=!1),$http.put("/api/v1/users/"+$scope.user._id,updateFavoriteObj)}}]);
...
"use strict";angular.module("users").service("ProfileImageService",["$http","Authentication",function($http,Authentication){var user=Authentication.user;this.getUploadedProfilePic=function(){var configObj={cache:!0};$http.get("api/v1/users/"+user._id+"/media/uploadedProfileImage/"+user.profileImageFileName,configObj).then(function successCallback(successCallback){console.log("profilePic",successCallback),user.profileImage="modules/users/client/img/profile/uploads/uploaded-profile-image.jpg"},function errorCallback(errorCallback){console.log("profile photo error",errorCallback),user.profileImage="modules/users/client/img/profile/default.png"})}}]);
