...
"use strict";angular.module("core").directive("footerDirective",["UtilsService",function(UtilsService){return{restrict:"AE",priority:0,templateUrl:"/modules/core/client/directives/views/footer-directive.html",controller:["$scope","$http",function($scope,$http){UtilsService.cssLayout(),$scope.create=function(){$http({method:"POST",url:"/api/signup",data:{email:$scope.email}}).success(function(data){console.log(data)}).error(function(err){console.log(err),err&&($scope.error_message="Please try again!")}),$scope.email=""}}]}}]);
...
"use strict";angular.module("users").controller("ChangeProfilePictureController",["$scope","$timeout","$window","Authentication","Upload","$http",function($scope,$timeout,$window,Authentication,Upload,$http){$scope.user=Authentication.user,$scope.imageURL=$scope.user.profileImageURL,$scope.uploading=!1,$scope.onFileSelect=function(files){if(files.length>0){$scope.uploading=!0;var filename=files[0].name,type=files[0].type,query={user:$scope.user,filename:filename,type:type},configObj={cache:!0};$http.post("api/v1/s3/upload",query,configObj).success(function(result){console.log("result v1\n",result),Upload.upload({url:result.url,transformRequest:function(data,headersGetter){var headers=headersGetter();return delete headers.Authorization,console.log("data v1\n",data),data},fields:result.fields,method:"POST",file:files[0]}).progress(function(evt){console.log("progress: "+parseInt(100*evt.loaded/evt.total))}).success(function(data,status,headers,config){$scope.uploading=!1,console.log("file "+config.file.name+"is uploaded successfully. Response: "+data),data&&data.s3Url&&(console.log("data v2\n",data),$scope.user.profileImageURL=data.s3Url,$scope.imageURL=data.s3Url),console.log("$scope.imageURL:\n",$scope.imageURL)}).error(function(){}),result&&result.s3Url&&(console.log("result v2\n",result),$scope.user.profileImageURL=result.s3Url,$scope.imageURL=result.s3Url)}).error(function(data,status,headers,config){$scope.uploading=!1})}}}]);
