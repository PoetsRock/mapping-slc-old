...
"use strict";angular.module("core").controller("ModalController",["$scope","$uibModalInstance","items",function($scope,$uibModalInstance,items){$scope.items=items,$scope.selected={item:$scope.items[0],toStateName:items.toStateName},$scope.selected.item?$scope.ok=function(){$uibModalInstance.close($scope.selected.item)}:$scope.ok=function(){$uibModalInstance.close($scope.selected.toStateName)},$scope.cancel=function(){$uibModalInstance.dismiss("user cancelled modal")}}]);
...
"use strict";angular.module("core").directive("mainMenu",["AdminAuthService",function(AdminAuthService){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/main-menu.html",controller:["$scope",function($scope){$scope.isAdmin=AdminAuthService.user,$scope.sidebar=L.control.sidebar("sidebar",{closeButton:!0,position:"left"}).addTo(map),$scope.sidebar.click=function(){L.DomUtil.hasClass(this,"active")?($scope.sidebar.close(),console.log("here i am close")):($scope.sidebar.open(this.firstChild.hash.slice(1)),console.log("here i am open"))}}],link:function($scope){}}}]);
...
"use strict";angular.module("core").directive("modalDirective",function(){return{restrict:"E",link:function(){$uibModal.open({animation:!0,templateUrl:template,controller:function($scope,$modalInstance,$location){$scope.stay=function(result){console.log("stay just a little bit longer, oh won't you stay"),$modalInstance.close(function(result){console.log("result: ",result)})},$scope.leave=function(){$scope.stay(),$location.path(toState)}},size:"lg"})}}});
...
"use strict";angular.module("core").directive("signInDirective",function(){return{restrict:"EA",templateUrl:"/modules/core/client/directives/views/sign-in-directive.html",controller:["$scope","Authentication",function($scope,Authentication){$scope.user=Authentication.user,""!==$scope.user&&("default.png"===Authentication.user.profileImageFileName||""===Authentication.user.profileImageFileName?$scope.user.profileImage="modules/users/client/img/profile/default.png":""!==Authentication.user.profileImageFileName?$scope.user.profileImage="modules/users/client/img/profile/uploads/uploaded-profile-image.jpg":ProfileImageService.getUploadedProfilePic())}]}});
